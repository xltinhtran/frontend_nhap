...
export function handleResumeSession(options) {
  ...
  const setId =
    typeof options === "object" ? options.setId || options.id : options;
...
export function navigateToLearnMode(setId, options = {}) {
  ...
  const actualSetId =
    typeof setId === "object" ? setId.setId || setId.id : setId;
...
export function initializeNewSession(set, mode) {
  ...
  cards.forEach((c) => {
    const rep = c.stats
      ? c.stats.repetitions !== undefined
        ? c.stats.repetitions
        : c.stats.Repetitions || 0
      : 0;
    if (rep > 0) {
