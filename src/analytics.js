...
export function recordCardStudy(correct) {
  ...
  // Update streak
  const { streakDays, lastStudyDate } = calculateStreak(
...
export function getTodayStats() {
  ...
  return {
    ...stats,
    accuracy:
      stats.cards > 0 ? Math.round((stats.correct / stats.cards) * 100) : 0,
  };
}
...
export function getTotalStats() {
  ...
  return {
    ...
    totalHours:
      Math.round(((analytics.totalSessionMinutes || 0) / 60) * 10) / 10,
  };
}
