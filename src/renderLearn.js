...
export function renderLearnQuestion(session, set, handlers) {
  ...
  if (selectedMode === "AUTO") {
    ...
    modeToPlay = availableModes.includes(selectedMode)
      ? selectedMode
      : "MULTIPLE_CHOICE";
  }
  ...
  newSpeakBtn.className =
    "mr-6 text-indigo-500 hover:text-indigo-700 flex-shrink-0 transition-all p-3 rounded-full hover:bg-indigo-100 shadow-md cursor-pointer transform hover:scale-110 active:scale-95";
...
function setupTextInput(card, handlers) {
  ...
  input.className =
    "w-full max-w-md px-4 py-3 text-lg border-2 border-slate-300 rounded-xl focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none text-center font-semibold text-slate-700 shadow-inner transition-colors";
...
function setupArrange(card, handlers) {
  ...
  words.forEach((word) => {
    ...
    pill.className =
      "px-4 py-2 bg-white border-2 border-slate-200 rounded-xl shadow-sm text-slate-700 font-bold hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-700 transition-all cursor-pointer transform hover:-translate-y-1 active:scale-95";
  ...
  btn.addEventListener("click", () => {
    ...
    const userSentence = Array.from(target.children)
    ...
    Array.from(target.children).forEach((p) => {
      p.disabled = true;
      if (isCorrect)
        p.classList.add("bg-green-100", "border-green-500", "text-green-700");
